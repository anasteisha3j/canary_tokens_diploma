...
def _int_env_port(default: int = 8080) -> int:
    """PORT / FLASK_RUN_PORT; на Windows 5000 часто зайнятий (AirPlay тощо)."""
...
class Config:
    ...
    APP_PORT = _int_env_port(8080)
    # Публічна URL програми (краще домен, не IP — інакше IP видно в посиланні)
    _pb = os.environ.get('PUBLIC_BASE_URL')
    PUBLIC_BASE_URL = (_pb if _pb else f'http://127.0.0.1:{APP_PORT}').rstrip('/')
